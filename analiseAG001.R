...
df_log <- df_log %>% filter(investimento_total_prestador > 0) %>% 
  mutate(investimento_total_prestador = log(investimento_total_prestador))

df_log <- df_log %>%
  filter(investimento_agua_prestador > 0) %>%
  mutate(investimento_agua_prestador = log(investimento_agua_prestador))


df_log <- df_log %>%
  filter(despesa_total_servico > 0) %>%
  mutate(despesa_total_servico = log(despesa_total_servico))

df_log <- df_log %>%
  filter(receita_operacional_direta_agua > 0) %>%
  mutate(receita_operacional_direta_agua = log(receita_operacional_direta_agua))
...
excluir <- c("ano", "sigla_uf", "id_municipio")

df_plot <- dplyr::select(df_log, -dplyr::all_of(excluir))

df_long_hist <- df_plot %>%
  pivot_longer(cols = -populacao_atendida_agua, names_to = "variavel", values_to = "valor")

ggplot(df_long_hist, aes(x = valor)) +
...
modelo_final <- lm(populacao_atendida_agua ~ investimento_total_prestador +
